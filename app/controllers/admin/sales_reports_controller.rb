  ...
  def index 
    if params[:format]
        ...
        respond_to do |format|
          format.csv {
             export_to_csv(@fromdate,@todate,@studio_id,@type)
          }
          format.html
        end
...
#         
      # else
        if !current_admin.studio.present?
          ...
          @type = "single"
          if params[:id] != current_admin.studio
            params[:id] = current_admin.studio
          end
          @studios = Studio.find(:all,:conditions=>["id=?",params[:id]])
          @current_selected = params[:id]
          
        end
      #end
  end
  
  def export_to_csv(fromdate,todate,studio_id,type)
    # begin
      filename = "#{RAILS_ROOT}/public/movie_fan_count.csv"
      
      if studio_id and studio_id.to_i != 0 and type != "all"
       # movies = Movie.find(:all,:conditions=>["studio_id = ?",studio_id])
        @studios = Studio.find(:all,:conditions=>["id=?",studio_id])
