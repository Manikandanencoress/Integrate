class Admin::Studios::SalesReportsController < ApplicationController
  ...
  def index
      ...
      params[:studio_id] = current_admin.studio_id if current_admin.studio_id != params[:studio_id] && params[:studio_id].blank?
  ...
  def export_to_csv(fromdate,todate,studio_id)
    ...
    @studios = Studio.find(:all,:conditions=>["id=?",studio_id],:order=>"name") if studio_id and studio_id.to_i != 0
